...
invrep(card_t c) {
    ...
    if(c != NULL && (c->num > 9u || c->color < 0 || c->type < 0 || c->color > 3 || c->type > 2)){
        valid = false;
    }
...
card_new(unsigned int num, color_t color, type_t s) {
    assert(num < 10 && (color == red || color == green || color == blue || color == yellow) && (s == normal || s == change_color || s == skip));
    card_t card = NULL;
...
card_samecolor(card_t c1, card_t c2) {
    return(card_get_color(c1) == card_get_color(c2));
}


bool
card_samenum(card_t c1, card_t c2) {
    return (card_get_number(c1) == card_get_number(c2));
}
